...
function inferRisks(intent, gitStatus) {
  const gitRisk = inferGitRisk(gitStatus, "generated context");
  ...
  return [
    ...
    inferGitRisk(gitStatus, "implementation scope"),
    "Generated prompt should be treated as a starting point, not an approval to make broad unrelated refactors."
  ];
}

function inferGitRisk(gitStatus, target) {
  if (gitStatus === "No git status available.") {
    return "This path is not a git repository, so diff-aware context is unavailable.";
  }

  if (gitStatus.startsWith("Working tree clean.")) {
    return "Working tree is clean; context is based on committed files and available diff information.";
  }

  return `Existing local changes may affect ${target}.`;
}
